...
class logisticRegression(nn.Module):
    def __init__(self, input, is_Image=False):
        super(logisticRegression, self).__init__()
        if is_Image:
            self.linear = imageRegression()
        else:
            self.linear = nn.Linear(input, 1)
...
class aipw(nn.Module):
    def __init__(self, n_covariates, is_Image=False):
        super(aipw, self).__init__()
        self.n_covariates = n_covariates
        if is_Image:
            self.linearRegression_0 = imageRegression()
            self.linearRegression_1 = imageRegression()
            self.logisticRegression = logisticRegression(self.n_covariates, is_Image=is_Image)
        else:
